...
from fastapi import FastAPI, WebSocket, Request, WebSocketDisconnect # fastAPI để xd + quản lý web
...
from urllib.parse import parse_qs # xử lý các URL 
from datetime import datetime
from concurrent.futures import ThreadPoolExecutor  # xử lý đa luồng
from deep_sort_realtime.deepsort_tracker import DeepSort 
import numpy as np # xử lý mảng và số học
from utils import * # lấy các hàm từ file utils.py
import json # xử lý dữ liệu json
import math
import traceback # xử lý thông tin in ra lỗi
...
os.makedirs(VIOLATION_FOLDER, exist_ok=True) # tạo thư mục lưu trữ nếu chưa có
...
def save_violation_to_db(timestamp, image_path, video_path, category, track_id):
    try:
        with sqlite3.connect(DATABASE_PATH) as conn:
            cursor = conn.cursor() #Tạo một đối tượng cursor để thực hiện các truy vấn SQL
            video_name = os.path.basename(video_path)
            cursor.execute('INSERT INTO violations (timestamp, image_path, video_path, category, track_id) VALUES (?, ?, ?, ?, ?)',
                           (timestamp, image_path, video_name, category, track_id))
...
@app.get("/", response_class=HTMLResponse) # chỉ định phản hổi là html
async def get_root(request: Request): # định nghĩa hàm xử lý bất đồng bộ 
...
@app.get("/api/violations_daily") # thống kê vi phạm từng ngày
...
@app.websocket("/ws/{video_name}")  # truyền dữ liệu video 
async def websocket_endpoint(websocket: WebSocket, video_name: str, roi_x1: int = 100, roi_y1: int = 100, roi_x2: int = 1150, roi_y2: int = 750):
    global active_websocket #sủ dụng hàm vừa tạo trên
    if active_websocket: # nếu có kết nối cũ
        await active_websocket.close() # thì đóng nó lại

    active_websocket = websocket # gán kết nối hiện tại 
    ...
    try:
        while cap.isOpened():
            ...
            original_frame = frame.copy()  # Sao chép frame gốc để sử dụng sau này
                    ...
                    for i, track in enumerate(current_track_vehicle):
                        ...
                        if crop_img.size != 0:
                            ...
                            for helmet_box in results_helmet.boxes:
                                ...
                                # Thêm tọa độ offset của ROI vào box của mũ bảo hiểm
            ...
            if detection_type == 'light':
                trang_thai_den = 0 # lưu trữ thông tin trạng thái đèn 
                den_phuong_tien = {} #lưu trữ trạng thái đèn tương ứng với mỗi phương tiện dựa theo track ID
                trang_thai_xe = {} 
                saverd_images = {}
                #re_phai = None
                ...
                light_frame = frame[light_roi_y1:light_roi_y2, light_roi_x1:light_roi_x2, :] # cắt vùng khoanh đèn
                ...
                result = model_vehicle.predict(roi, conf=0.35, verbose=False) # load model
                if len(result): # xem resuilt có kết quả không
                    result = result[0] # resuil là một danh sách, lấy giá trị đầu tiên 
                    names = result.names # lấy danh sách các class trong mảng
                    detect = [] # khởi tạo danh sách lưu thông tin bounding box
                    track_image_count = {} # khởi tạo từ điển để theo dõi số lần phát hiện mỗi phương tiện trong các khung hình
                    for box in result.boxes: # duyệt qua từng bouding box phát hiện được
                        x1, y1, x2, y2 = list(map(int, box.xyxy[0])) # lấy tọa độ 
                        conf = box.conf.item() # lấy độ tin cậy
                        cls = int(box.cls.item()) # lấy class
                        detect.append([[x1, y1, x2 - x1, y2 - y1], conf, cls]) # lưu thông tin vào danh sách
                    #----------------------------------------------------
                    
                    def xoa_data_vi_pham(track_id, xoa_du_lieu): # khởi tạo hàm xóa
                        try:
                            with sqlite3.connect(DATABASE_PATH) as conn: # kết nối csdl
                                cursor = conn.cursor()
                                if xoa_du_lieu == 'co_xoa':
                                    ...
                                    for row in rows: # lặp qua từng kết quả
                                        image_path = row[0] # lấy đường dẫn ảnh
                                        if os.path.exists(image_path): # nếu ảnh có tồn tại
                    ...
                    def luu_anh_vi_pham(frame, frame_with_box, track, label, track_id, roi_x1, roi_y1, output_folder):
                        ltrb = track.to_ltrb() # lấy tọa độ hộp giới hạn của track
                        x1, y1, x2, y2 = list(map(int, ltrb)) # chuyển tọa độ sang dạng int
                    ...
                    tracks = track_light.update_tracks(detect, frame=roi) # lấy các track trong frame hiện tại
                    for i, track in enumerate(tracks):
                        if track.is_confirmed() and track.det_conf: # nếu track tồn tại và độ tin cậy cao
                            ltrb = track.to_ltrb() # lấy tọa độ của track dạng left top right bottom
                            x1, y1, x2, y2 = list(map(int, ltrb)) # chuyển sang kiểu int
                            yc = roi_y1 + (y1 + (y2 - y1) // 2)  # tính tâm theo trục OY, tính từ trên xuống
                            xc = roi_x1 + (x1 + (x2 - x1) // 2)  # tính tâm theo trục OX, tính từ trái sang phải
                            track_id = track.track_id # lấy ID của track
                            
                            if track_id not in trang_thai_xe: # nếu track chưa có trạng thái xe
                                trang_thai_xe[track_id] = 0   # khởi tạo trạng thái là 0
                            current_state = trang_thai_xe.get(track_id, 0) 
                            # lấy trạng thái hiện tại của track, nếu ko có thì gán là o
                            
                            label = labels.get(track_id, None) # lấy nhãn của track, ko có thì gán mặc định là 0
                            xoa_du_lieu = None # khởi tạo xóa dữ liệu là None
                            ...
                            if label: # nếu track được gán nhãn
                                frame_with_box = original_frame.copy() # lấy frame
                                color = (0, 0, 255) if label == 'vuot' else (0, 255, 0) # lấy màu với nhãn
                                label_with_id = f"({label}, ID: {track_id} {trang_thai_xe[track_id]})" # lấy nhãn hiển thị
                                color = (0, 0, 255) if label == 'vuot' else (0, 255, 0)
                                cv2.rectangle(roi, (x1, y1), (x2, y2), color, 2) # khoanh khung 
                                # vẽ khung bounding box
                                cv2.rectangle(roi, (x1 - 1, y1 - 20), (x1 + len(label_with_id) * 8 + 3, y1), color, -1) 
                                # vẽ vùng hiển thị nhãn
